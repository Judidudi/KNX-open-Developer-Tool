...
// udev rules for KNX USB HID interfaces:
//   hidraw   – direct USB mode (our UsbKnxInterface)
//   usb      – libusb access for knxd 0.14.x
static const char kUdevRuleContent[] =
    "# KNX USB HID interfaces – auto-generated by KNX open Developer Tool\n"
    "SUBSYSTEM==\"hidraw\", ATTRS{idVendor}==\"135e\", MODE=\"0666\"\n"
    "SUBSYSTEM==\"hidraw\", ATTRS{idVendor}==\"0681\", MODE=\"0666\"\n"
    "SUBSYSTEM==\"hidraw\", ATTRS{idVendor}==\"16d0\", MODE=\"0666\"\n"
    "SUBSYSTEM==\"hidraw\", ATTRS{idVendor}==\"03eb\", MODE=\"0666\"\n"
    "SUBSYSTEM==\"usb\", ATTRS{idVendor}==\"135e\", MODE=\"0666\"\n"
    "SUBSYSTEM==\"usb\", ATTRS{idVendor}==\"0681\", MODE=\"0666\"\n"
    "SUBSYSTEM==\"usb\", ATTRS{idVendor}==\"16d0\", MODE=\"0666\"\n"
    "SUBSYSTEM==\"usb\", ATTRS{idVendor}==\"03eb\", MODE=\"0666\"\n";

static bool installKnxUdevRule(QWidget *parent)
{
    QTemporaryFile tmp(QDir::tempPath() + QStringLiteral("/knx-usb-XXXXXX.rules"));
    if (!tmp.open()) return false;
    tmp.write(kUdevRuleContent);
    tmp.flush();
    tmp.setAutoRemove(false);

    const QString cmd = QStringLiteral(
        "cp %1 /etc/udev/rules.d/99-knx-usb.rules "
    ...
    const int rc = QProcess::execute(
        ...
        {QStringLiteral("sh"), QStringLiteral("-c"), cmd});
    QFile::remove(tmp.fileName());
    if (rc != 0) {
        QMessageBox::warning(parent,
            QObject::tr("Einrichtung fehlgeschlagen"),
            QObject::tr("Die udev-Regel konnte nicht installiert werden (Fehlercode %1).\n"
                        "Bitte führen Sie folgenden Befehl manuell aus:\n\n"
                        "sudo cp /tmp/99-knx-usb.rules /etc/udev/rules.d/\n"
                        "sudo udevadm control --reload-rules").arg(rc));
    }
    return rc == 0;
}
    ...
    : QMainWindow(parent)
{
    ...
    if (KnxdManager::isInstalled()) {
        ...
        connect(m_knxdManager, &KnxdManager::errorOccurred,
                this, &MainWindow::onInterfaceError);
        connect(m_knxdManager, &KnxdManager::udevSetupNeeded,
                this, &MainWindow::onKnxdUdevNeeded);
...
void MainWindow::onConnectClicked()
{
    ...
    ConnectDialog dlg(m_usbMonitor, this);
    if (dlg.exec() != QDialog::Accepted)
        return;
...
void MainWindow::onUsbDeviceFound(KnxUsbMonitor::DeviceInfo info)
{
    if (!info.accessible) {
        if (!m_udevPromptShown) {
            m_udevPromptShown = true;
            const auto btn = QMessageBox::question(
                this,
                tr("KNX-Interface gefunden"),
                tr("Das KNX-Interface \"%1\" wurde erkannt, kann aber nicht "
                   "geöffnet werden (fehlende Zugriffsrechte).\n\n"
                   "Zugriffsregel einmalig einrichten? "
                   "(erfordert Administrator-Passwort)")
                    .arg(info.name),
                QMessageBox::Yes | QMessageBox::No);
            if (btn == QMessageBox::Yes)
                installKnxUdevRule(this);
        }
        return;
    }
...
void MainWindow::onUsbDeviceRemoved(QString path)
{
...
void MainWindow::onKnxdUdevNeeded()
{
    if (m_udevPromptShown) return;
    m_udevPromptShown = true;
    const auto btn = QMessageBox::question(
        this,
        tr("KNX-Interface: USB-Berechtigung fehlt"),
        tr("knxd benötigt Schreibzugriff auf das USB-Gerät.\n\n"
           "Zugriffsregel einmalig einrichten?\n"
           "(erfordert Administrator-Passwort)"),
        QMessageBox::Yes | QMessageBox::No);
    if (btn == QMessageBox::Yes)
